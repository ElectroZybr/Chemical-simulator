...
World::World(glm::vec3 size, glm::vec3 renderOffset) : size(size), renderOffset(renderOffset), grid(size), vectorField_(glm::ivec3(size), 0) {
    ...
    if (!state_.forceField_.setForceField("classic_md")) {
        Logger::error("World", "Failed to select default force field 'classic_md'");
    }
    else if (state_.forceField_.activeForceField() == nullptr) {
        Logger::warning("World", "Default force field 'classic_md' is selected but not available");
    }
    else {
        Logger::ok("World", "Default force field selected: {}", state_.forceField_.getForceField());
    }
...
void World::updateVectorField() {
    ...
    if (forceField == nullptr) {
        Logger::warning("World", "Vector field update skipped: no active force field");
    ...
    vectorFieldDirty_ = false;
    Logger::info("World", "Vector field updated for force field {}", state_.forceField_.getForceField());
}
