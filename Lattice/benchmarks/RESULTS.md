...
# ПОСЛЕ MERGE UPSTREAM/MAIN — glm + Morton-гейт (merge `46e8df1b`)

Слияние ушедшего вперёд upstream/main (glm-векторы, разбивка рендера на файлы, Morton-сортировка
атомов по ячейкам, octree-Кулон) поверх форк-оптимизаций. Полная история конфликтов и адаптаций —
в сообщении мерж-коммита; здесь только перф-итог.

## Morton-сортировка — гейт по числу атомов

upstream добавил Morton-сортировку атомов по ячейкам в `NeighborList::rebuildPipeline` (улучшение
локальности кэша). Замеры выявили: это СЕРИЙНЫЙ radix-sort + remap всех per-atom массивов,
стоимость растёт с N и не параллелится. Изоляция (тот же слитый код, сортировка вкл/выкл,
High-priority, медиана 5):

| 1M атомов | Morton вкл | Morton выкл |
|---|---|---|
| 1 поток | 409 ms (2.4 шаг/с) | 88 ms (11.3 шаг/с) |
| все 28 ядер | 345 ms (2.9 шаг/с) | 19 ms (52 шаг/с) |

Сортировка добавляет ~320 ms/rebuild на 1M; на all-core серийный sort доминирует над parallelized
шагом (Amdahl) — провал 16x. Выигрыш же ничтожен: на 104k 1 поток лишь +0.6 ms, all-core — в
пределах шума. Поэтому Morton гейтится по `kMortonSortThreshold = 200000`: сортируем только
малые/средние сцены, большие параллельные не платят серийный sort. Порог эмпирический, тюнится.
BaselineParity-сцена (27 атомов) ниже порога → Morton там активен, golden переснят под этот порядок.

## Полный шаг, слитый код с гейтом (i7-14700K, High-priority, медиана 5)

| N атомов | CPU 1 ядро | CPU все 28 ядер |
|---|---|---|
| 103 823 | 12.1 ms (83 шаг/с) | 4.0 ms (250 шаг/с) |
| 1 000 000 | 88 ms (11.3 шаг/с) | 19 ms (52 шаг/с) |

Числа не хуже до-мерж замеров на всех N (1M даже заметно быстрее). ВНИМАНИЕ о сравнимости:
upstream сменил bench-прогрев (`warmupScene`), поэтому абсолютное сравнение с до-мерж числами выше
по файлу приблизительно — чистая изоляция эффекта только within-harness (Morton вкл/выкл выше).
GPU-режим (zero-copy) проверен визуально (GUI smoke) после переноса в `RenderAtoms.cpp`.

Провенанс: `FullStepWithNeighborList` (`Lattice/benchmarks/physics/BM_SimulationStep.cpp`), env
`LL_TBB_THREADS=1` для 1 ядра; гейт — `NeighborList::rebuildPipeline` (`kMortonSortThreshold`);
корректность — 36/36 юнит-тестов. Сборка `cmake --preset bench`.
