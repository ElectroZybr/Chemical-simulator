...
foreach(line IN LISTS csv_lines)
    ...
    if(key STREQUAL "")
        # Строка не разобралась в 3 поля (key;en;ru) — почти всегда лишняя «;» ВНУТРИ значения
        # (разделитель внутри текста). Молчком пропускать нельзя: перевод тихо исчезает и UI
        # показывает сырой ключ (так было с imgui_gpu_physics_lj_only). Предупреждаем громко.
        message(WARNING "translate.csv: строка пропущена, ожидалось 3 поля через ';' (лишняя ';' внутри значения?): ${line}")
        continue()
    endif()
