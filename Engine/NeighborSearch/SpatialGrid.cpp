...
SpatialGrid::SpatialGrid(const Vec3f& worldSize, float cellSize) : cellSize(cellSize) {
    ...
    const float cellsX = std::max<float>(1.f + 2.f * kGhostLayers, (size.x + this->cellSize - 1.f) / this->cellSize + 2.f * kGhostLayers);
    const float cellsY = std::max<float>(1.f + 2.f * kGhostLayers, (size.y + this->cellSize - 1.f) / this->cellSize + 2.f * kGhostLayers);
    const float cellsZ = std::max<float>(1.f + 2.f * kGhostLayers, (size.z + this->cellSize - 1.f) / this->cellSize + 2.f * kGhostLayers);
